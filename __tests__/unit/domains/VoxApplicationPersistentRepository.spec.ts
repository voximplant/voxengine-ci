...
describe('VoxApplicationPersistentRepository', () => {
  ...
  describe('create', () => {
    ...
    it('should call createFile with arguments', async () => {
      await voxApplicationPersistentRepository.create({applicationName: 'application'});
      createFileStub.restore();
      assert.calledWith(createFileStub,'applications/application', 'application.config', JSON.stringify({applicationName: 'application'}));
    });
  });
  ...
  describe('createMetadata', () => {
    ...
    beforeEach(function() {
      createMetadataFileStub = stub(fileSystemContext.client, 'createMetadataFile');
    });
    it('should call createMetadataFile with arguments', async () => {
      await voxApplicationPersistentRepository.createMetadata({applicationName: 'application', applicationId: 11});
      createMetadataFileStub.restore();
      assert.calledWith(createMetadataFileStub,'applications/application', 'application.metadata.config', JSON.stringify({applicationName: 'application', applicationId: 11}));
    });
  });
